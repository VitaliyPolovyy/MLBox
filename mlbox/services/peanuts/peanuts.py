...
def process_image(input_image_file):
    ...
    preprocessed_image.save(result_folder / f"{input_image_file.stem}_preprocessed_image{input_image_file.suffix}")
    ...
    for i in range(len(detection_result[0].boxes)):
        ...
        detection_result[0].boxes[i]['class_accuracy'] = round(random.uniform(0.5, 1.0), 2)

    # Step 4: Prepare results
    result_table, data, result_image = prepare_result(detection_result, pixels_per_mm, input_photo=preprocessed_image)

    # Step 5: Create the Excel report
    excel_file = prepare_excel(result_table, data, result_image)

    # Step 6: Send results to ERP (optional)
    send_result_to_ERP(excel_file)

    return "Processing complete"

def preprocessing_image_for_detection(input_photo):
    def rotate_image(image: np.ndarray, rect_points: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def crop_and_resize(image: np.ndarray, rect_points: np.ndarray, target_width: Optional[int], padding_percent: float) -> np.ndarray:
...
def peanut_detection(image):
    ...
    if results and len(results) > 0 and results[0].masks is not None:
        for i, (box, mask) in enumerate(zip(results[0].boxes.xyxy, results[0].masks.data)):
            ...
            detection_item = {
                ...
                'class_accuracy': round(random.uniform(0.5, 1.0), 2),  # Placeholder for accuracy
                'minor_diameter': ellipse_params['minor_diameter'] if ellipse_params else 0,
                'major_diameter': ellipse_params['major_diameter'] if ellipse_params else 0,
...
def prepare_excel(result_table, data, result_image):
    ...
    with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
        ...
        summary_df.to_excel(writer, sheet_name='Peanut Analysis', index=True, startcol = len(result_table.columns) + 2)
...
def send_result_to_ERP(excel_file: Path):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to send the result to ERP. Status code: {response.status_code}, Response: {response.text}")
