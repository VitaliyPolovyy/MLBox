...
from utils.cvtools import masks_to_coco_annotations
...
def save_image_with_result(image, detected_ellipses, grid_points, image_path, output_path):
    ...
    for obj in detected_ellipses:
        ...
        color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255), 100)  # Add alpha for transparency
        overlay = image_with_all_ellipses.copy()
        # Draw and fill the ellipse on the overlay
        cv2.ellipse(overlay, ellipse, color[:3], -1)  # Use only RGB values for OpenCV drawing
        # Add transparency to the filled ellipse
        cv2.addWeighted(overlay, 0.4, image_with_all_ellipses, 0.6, 0, image_with_all_ellipses)
        # Draw the outline of the ellipse
        cv2.ellipse(image_with_all_ellipses, ellipse, (0, 0, 0), 2)  # Draw the ellipse outline in black
        # Draw the point as a cross
        cross_size = 5
        cv2.line(image_with_all_ellipses, (point[0] - cross_size, point[1]), (point[0] + cross_size, point[1]), (0, 0, 255), 2)
        cv2.line(image_with_all_ellipses, (point[0], point[1] - cross_size), (point[0], point[1] + cross_size), (0, 0, 255), 2)
        # Display the score and accuracy on the image
        center = (int(ellipse[0][0]), int(ellipse[0][1]))
        cv2.putText(image_with_all_ellipses, f'Score: {score:.2f}, Acc: {accuracy:.2f}', center, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)

    for processed_point in grid_points:
        cv2.circle(image_with_all_ellipses, processed_point, radius=3, color=(0, 255, 0), thickness=-1)  # Draw a green filled circle

    for processed_point in grid_points:
        cv2.circle(image_with_all_ellipses, processed_point, radius=3, color=(0, 0, 255), thickness=-1)  # Draw a green filled circle
...
def detect_ellipses_sam2(image_path=None, image_data=None, predictor=None, grid_points=None,
                         min_diameter_of_object_px=None, max_diameter_of_object_px=None,
                         accuracy_of_reliability=0.6, threshold_value=200, ellipse_accuracy=0.9):
    ...
    processed_points = []
    objects = []  # List to store suitable ellipses along with their masks, accuracy, and score
    combined_mask = np.zeros_like(gray_image, dtype=np.float32)  # Combined mask for all objects
    ...
    for idx, point in enumerate(tqdm(grid_points, desc="Processing grid points")):
        ...
        point_contained = any(
            point[1] < existing_mask["mask"].shape[0] and
            point[0] < existing_mask["mask"].shape[1] and
            existing_mask["mask"][point[1], point[0]]
        ...
        masks, scores, _ = predictor.predict(point_coords=input_points, point_labels=point_labels,
        ...
        if len(scores) > 0 and scores[0] > accuracy_of_reliability:
            ...
            if not (math.pi * (min_diameter_of_object_px / 2) ** 2 <= mask_area <= math.pi * (
                    max_diameter_of_object_px / 2) ** 2):
                continue
            ...
            if overlap_area < 0.1:
                ...
                if contours:
                    ...
                    if len(contour) >= 5:  # At least 5 points are needed to fit an ellipse
                        ...
                        if min_diameter_of_object_px <= minor_diameter <= max_diameter_of_object_px and min_diameter_of_object_px <= major_diameter <= max_diameter_of_object_px:
                            ...
                            accuracy = ellipse_area / contour_area if contour_area > 0 else 0

                            if ellipse_accuracy <= accuracy <= 2 - ellipse_accuracy:
                                # Store the ellipse, mask, accuracy, and score in the list
                                objects.append({'ellipse': ellipse, 'mask': mask, 'accuracy': scores[0], 'score': score,
                                                'point': point})
...
def process_all_images_in_folder(folder_path):
    sam2_path = sam2_checkpoint = os.path.abspath(r"C:\My storage\Python projects\MLBox\models\sam2.1")
    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith(('.jpg', '.jpeg', '.png')):
            try:
                ...
                coco_output_path = os.path.join(result_folder_path, f"{os.path.splitext(filename)[0]}_coco.json")
                ...
                grid_points = [(x, y) for y in range(0, height, step_size)
                ...
                detected_ellipses = detect_ellipses_sam2(
                    ...
                    max_diameter_of_object_px=10 * MIN_SIZE_OF_OBJECT_MM * pixels_per_mm,
                ...
                coco_json = masks_to_coco_annotations(masks=all_masks,
                                          image_id=1,
                                          filename=filename,
