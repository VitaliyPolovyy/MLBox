...
if os.getenv('VIRTUAL_ENV') is None or 'cv_env' not in os.getenv('VIRTUAL_ENV'):
    raise EnvironmentError("This script must be run in the 'cv_env' virtual environment")
...
class MLBox:
    ...
    async def send_result_to_erp(self, image_data: bytes, json_data: dict):
        ...
        try:
            ...
            files = {
                'image': ('processed_image.png', image_data, 'image/png')  # The image as a file in form-data
            }
