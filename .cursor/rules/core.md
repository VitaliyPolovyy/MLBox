...
6\.  \*\*External verification\*\* (official docs, but \_always\_ verify against the actual implementation).
...
\# Assumes SOME\_TOKEN is set and makes an API call.

Make an API call using the SOME\_TOKEN environment variable.
...
\# 2. Confirm that SOME\_TOKEN is present and has a value.

\# 3. Only then, make the API call using SOME\_TOKEN.
...
\- \*\*REQUIRED\*\*: Understand the system as it \*\*IS\*\*, not as documentation says it \_should\_ be. The code is the ultimate source of truth.
...
\- ✅ "Analyzing a similar implementation in `\[file\_path]` to understand the pattern..."
